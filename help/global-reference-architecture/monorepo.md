...
description: Leer hoe u de monorepo-benadering kunt gebruiken voor globale referentiearchitectuur om een schaalbare en veerkrachtige handelservaring tot stand te brengen
jira: KT-16728
doc-type: tutorial
audience: all
...
# Monorepo Global Reference Architecture-patroon
...
De ontwikkeling vindt plaats in de map packages. Wanneer u `packages` uitvoert, worden koppelingen naar de pakketten in de map `vendor` gemaakt in de map `composer update` . Op deze manier wordt de code onderdeel van de Adobe Commerce-installatie.
